...
#x_c=x-p*sym.sin(theta)
#y_c=y+p*sym.cos(theta)
#_x=x_c-R*sym.sin(theta)-x_m
#_y=y_c+R*sym.cos(theta)-y_m
## Contact Point 1: The right finger: surface contact with curvy side
#phi_2= s_m - (_x*sym.sin(theta_m)-_y*sym.cos(theta_m))
#psi_m= _x * sym.sin(theta_m) + _y * sym.cos(theta_m)
#psi_o= R * (theta_m-theta)
#psi_2=psi_m-psi_o
#_b=psi_m-p*sym.sin(theta_m-theta)
#J_2n=np.array([-sym.sin(theta_m),sym.cos(theta_m),-_b]).reshape(3,1)
#_a=(x_m-x)*sym.sin(theta)-(y-y_m)*sym.cos(theta)
#J_2t=np.array([sym.cos(theta_m),sym.sin(theta_m),s_m+_a]).reshape(3,1)
#J_2=np.hstack((J_2n,J_2t))
#C2=contact_point_symbolic_2D(mysystem,phi=phi_2,psi=psi_2,J=J_2,friction=0.5,name="contact point")
#C2.sliding=False
##C2.sticking=False

# Contact Point 2: The left finger: point contact with flat side
...
Eta_1=environment(0)
Eta_1.dict_of_values={x:0,y:R-p,theta:0.1,x_m:0.18,y_m:0.82,theta_m:np.pi/2.0-0.3,s_m:0.8,
...
right_shift=0
theta_shift=0.5
x_goal=np.array([0,R-p,theta_shift,0.18,0.82,np.pi/2-0.3,0.8,0,0,0]).reshape(10,1).reshape(10,1)
    ...
    for t in range(T+1):
        ...
        if t!=T:
            ax.set_title(r'%d/%d %s'%(t,T,mysystem.name)+'\n'+
                         r'$\lambda^{ground}_n: %0.1f * \lambda^{ground}_n: %0.1f $'
                         %(u_lambda[t][0],u_lambda[t][2])
                         +'\n'+r'$\lambda^{rF}_t: %0.1f * \lambda^{rF}_t: %0.1f $'
                         %(u_lambda[t][1],u_lambda[t][3])
                         +'\n'+r'$\lambda^{lF}_t: %0.1f * \lambda^{lF}_t: %0.1f $'
